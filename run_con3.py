...
def prepare_dataset(examples, tokenizer, max_length, model_name):
    ...
    for f in features:
        input_ids1.append(f[0].squeeze(0))
        attention_mask1.append(f[1].squeeze(0))
        input_ids2.append(f[2].squeeze(0))
        attention_mask2.append(f[3].squeeze(0))
        labels.append(torch.tensor(f[4], dtype=torch.long))
        task_ids.append(torch.tensor(f[5], dtype=torch.long))
        code_pairs.append((f[6], f[7]))  # 👈 code1, code2
...
def main():
    ...
    if args.do_train:
        ...
        train_dataset, train_code_pairs = prepare_dataset(train_examples, tokenizer, args.max_source_length, model_name)
       ...
       #nb_tr_examples, nb_tr_steps,global_step,best_bleu,best_loss = 0,0,0,0,1e6
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(bar):
                ...
                if hasattr(train_sampler, 'indices'):
                    batch_indices = train_sampler.indices[step * args.train_batch_size : (step + 1) * args.train_batch_size]
                else:
                    batch_indices = list(range(step * args.train_batch_size, (step + 1) * args.train_batch_size))

                # 精准恢复这组 batch 对应的 code1, code2
                debug_batch = [train_code_pairs[i] for i in batch_indices]  
                
                sen_vec1 = get_sentence_embedding(model, source_ids, source_mask, model_name, tokenizer,debug_batch)
                #print("First vector done", flush=True)
                sen_vec2 = get_sentence_embedding(model, target_ids, target_mask, model_name, tokenizer,debug_batch)
            #print("Second vector done", flush=True)
            #print("task_ids4:", [t.item() for t in task_ids],flush=True)
                loss_temp = torch.zeros((len(sen_vec1), len(sen_vec1) * 2 - 1), device=args.device)
            #print("task_ids3:", [t.item() for t in task_ids],flush=True)
